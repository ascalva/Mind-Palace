...
## Current phase
> Phase: 5 COMPLETE (2026-06-25) — verified against gate (120 passed: 114 logic + 6 live; 5 podman skipped, machine off). Dynamic agent factory in `core/factory/` + `ops/gate.py`: base role library (§9), scope ceiling `role.scope ∩ PRE_DECLARED_MAX={run_python}` (no shell/cred/net tool exists → unmintable), object-capability `ToolDispatcher` (out-of-scope = unreachable), `MintedAgent` (advisory `respond`+self-eval SEPARATE from action `invoke`; Inv 3), beyond-scope/privileged → `HumanGate` seam (Inv 5), SQLite persisted-agent registry. ⚠️ Phase 4 empirical `-m podman` still pending — podman machine won't boot here (libkrun→emergency mode; applehv wedged after churn); see `docs/runbook.md` → "Sandbox runtime". Next: Phase 6 (Interface layer, Zone B — gateway + local/Tailscale adapter). Update this line as phases complete.
```
