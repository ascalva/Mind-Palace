...
| ------------------- | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `scope-guard`       | PreToolUse: Edit\|Write\|MultiEdit                                                          | Read active plan id from `.claude/state/active-plan`; parse `write_scope` globs from plan front-matter; deny out-of-scope `file_path` with a reason string fed back to the agent. A global foundation-file denylist
  (`CONSTITUTION.md`, `eval/golden/**`, `eval/golden.py`) applies beneath any plan, in every session, orchestrator included. Design notes are guarded by STATUS, not location
  (A8): draft notes are agent-writable working material; ratified/superseded notes are agent-immutable — enforced pre-hoc on on-disk status and post-hoc against HEAD status
  (laundering-proof).                                                                                                                                                                                                                         |
...
**Status-value normalization — part of the blessing-detection contract (A5, warrant: finding-0006).** All three blessing detectors — pre-hoc `gate-guard` and both Stop-gate paths (tracked and untracked) — compare a plan/note's `status` value by exact string equality against `ratified` / `ready`. The parser must therefore **normalize the status value before comparison**: strip a trailing YAML comment (cut at the first ` #` — space-hash, matching YAML comment semantics — then rstrip), scoped to the _status extraction path only_, not every scalar (a `#` may be legitimate in other fields). Without this, a comment-bearing status line — `status: ready   # note` — parses to the value `ready   # note`, which `!= "ready"`, and **every** detector silently fails to recognize the blessing: an enforcement-invisible blessing that defeats even the pre-hoc gate 0005 left intact. The failure is one-directional (false-negative in the blessing direction only — a `proposed # …` line is still correctly not a blessing), so it silently _weakens_ the bright line, the worst failure direction for a guardrail. The no-space case `ready#x` is deliberately _not_ stripped (YAML requires the space), so it remains a non-blessing — normalization only ever refuses to recognize a malformed blessing, never fabricates one. The build-plan template must not model the comment-on-status-line pattern (its own line 4 originally did, which is how 0006 surfaced): minted status lines are clean.
