...
- **G4 — Drift metric & tolerance. [CLOSED — forward item A1]** $D(t)=d(\mu(s_t),B)$ needs a concrete *normalized* metric over a mixed profile (rates ⊕ conformance vector) and a chosen $\Theta$. → `eval.drift`: $\mu$ = the golden-set capability rates (recall↑, overlap↑, mean_distance↓) ⊕ the Constitution-fingerprint conformance signal; $d$ = **one-sided L2 deterioration distance** — each axis contributes only its bad-direction deviation past baseline, normalized by a blessed per-axis tolerance, combined by L2 (so healthy improvement is $0$ drift, not flagged), and a Constitution-fingerprint breach **hard-trips** ($D=\infty$). $\Theta=1.0$ is a **human-set frozen fixed point** blessed in `eval/golden/baseline.json` (excluded from the lever set; F4-calibrated then re-blessed). The gate's drift conjunct is now this real gauge (`ops.selfmod.build_golden_validator` → `drift_from_report`), replacing the rolling-regression stand-in. *(A2 extends $\mu$ with structural axes — `Axis` is additive; F4 calibrates $\Theta$ on observed curves.)*
