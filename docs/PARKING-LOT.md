...
| CQ-scope | **the capability-scope type system (the UNIFIED QUERY LANGUAGE)** — all Views as instances of one bounded-lattice scope algebra | CQ §2.1 | **GRADUATED → `bp-039` (proposed)** (note RATIFIED by owner 2026-07-15, `3f5591d`; graduated same session) | → owner blesses `bp-039` `proposed→ready` → `/build` (opus, self-driven) | **`dn-capability-scope` RATIFIED**; `bp-039 CQ-scope` proposed — `core/scope.py` pure typing layer: `Scope` lattice (meet/join/⊑), `WorldReach` NONE-floor (reconciles the note's `NONE<SENSING` vs code's `ReversibilityClass`, bridge ops-side), partial clock T-meet (constructor error on no common clock), firewall ideals, `req()` as a DECLARATION on the five Views (bit-identical-reads falsifier), Inv/Rate markers (Item 4, deferrable). Parked in-plan: `factory.grant` wiring, N materialization (CS-a), sensor dual (CS-c), ObservedView/DreamsView scope |
